...
import { Workshop, WorkshopDocument } from 'src/workshops/workshop.schema';
  ...
  async chatbotExtraResponse(
    ...
    if (label) {
      ...
      if (label.indexOf('coach') > -1) {
        const student: Student | null = await this.studentModel.findOne({
          username: username,
        });
        if (!student) throw new NotFoundException('Student not found')
        const coach: Staff | null = await this.staffModel.findOne({
          username: student.coach,
        });
        if (!coach) throw new NotFoundException('Coach not found')
        const coachFullName =
          locale == 'hy'
            ? `${coach.firstName.am} ${coach.lastName.am}`
            : `${coach.firstName.en} ${coach.lastName.en}`;
